-- WayCrew v0.3.3 - robust route geometry transport for create_activity.
-- Replaces the jsonb RPC argument with explicit JSON text parsing to avoid
-- route geometry being lost/decoded inconsistently by clients/PostgREST.

-- Remove v0.3.0 create_activity signature.
...
create or replace function public.create_activity(
  ...
  p_route_geojson_text text default '',
...
begin
  ...
  begin
    v_route_geojson := nullif(trim(coalesce(p_route_geojson_text, '')), '')::jsonb;
  exception when others then
    raise exception 'route_geometry_invalid_json';
  end;
  if v_route_geojson is null
     or v_route_geojson->>'type' <> 'LineString'
     or jsonb_typeof(v_route_geojson->'coordinates') <> 'array'
     or jsonb_array_length(v_route_geojson->'coordinates') < 2 then
    raise exception 'route_geometry_required';
  end if;
  ...
  insert into public.activity_routes(
    ...
    v_activity_id, v_route_label, v_route_geojson, round(p_route_distance_km::numeric, 2),
...
revoke all on function public.create_activity(
  text,text,text,text,text,timestamptz,text,text,double precision,double precision,
  text,text,double precision,double precision,text,text,double precision,double precision,
  text,double precision,integer,text,text,jsonb,integer,text,text,uuid
) from public;
